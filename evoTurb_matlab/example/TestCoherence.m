...
plot(f_est,abs(mean(CPSD_1_1_1_to_1_1_2,1).^2)./mean(PSD_1_1_1,1)./mean(PSD_1_1_2,1));
plot(f_est,abs(mean(CPSD_1_1_1_to_1_1_3,1).^2)./mean(PSD_1_1_1,1)./mean(PSD_1_1_3,1));
% plot(f_est,abs(mean(CPSD_1_1_2_to_1_1_3,1).^2)./mean(PSD_1_1_2,1)./mean(PSD_1_1_3,1));
plot(ConfigParameters.f,squeeze(Cohx_u(1,2,:)).^2)
plot(ConfigParameters.f,squeeze(Cohx_u(1,3,:)).^2)
legend('estimated: Dx = 50 m','estimated: Dx = 100 m est','theoretical: Dx = 50 m','theoretical: Dx = 100 m')
set(gca,'Xscale','log')
xlabel('frequency [Hz]')
...
subplot(2,2,2)
hold on; grid on; box on;
plot(f_est,abs(mean(CPSD_1_1_1_to_1_4_1,1).^2)./mean(PSD_1_1_1,1)./mean(PSD_1_4_1,1));
plot(f_est,abs(mean(CPSD_1_1_1_to_1_8_1,1).^2)./mean(PSD_1_1_1,1)./mean(PSD_1_8_1,1));
if strcmp(ConfigParameters.TurbModel,'Kaimal')   
    ...
    plot(f_est,yz_coh1.^2);
    plot(f_est,yz_coh2.^2);
    set(gca,'Xscale','log')
    xlabel('frequency [Hz]')
    ylabel('magnitude coherence [-]')
    title('Dz = 0')
    legend('estimated: Dy = 16 m','estimated: Dy = 32 m','theoretical: Dy = 16 m','theoretical: Dy = 32 m')
else
    ...
    ylabel('magnitude coherence [-]')
    title('Dz = 0')
    legend('estimated: Dy = 12 m','estimated: Dy = 28 m','theoretical: Dy = 12 m','theoretical: Dy = 28 m')
end    

subplot(2,2,3)
hold on; grid on; box on;
plot(f_est,abs(mean(CPSD_1_4_1_to_1_1_2,1).^2)./mean(PSD_1_4_1,1)./mean(PSD_1_1_2,1));
plot(f_est,abs(mean(CPSD_1_4_1_to_1_1_3,1).^2)./mean(PSD_1_4_1,1)./mean(PSD_1_1_3,1));
plot(f_est,abs(mean(CPSD_1_8_1_to_1_1_2,1).^2)./mean(PSD_1_8_1,1)./mean(PSD_1_1_2,1));
plot(f_est,abs(mean(CPSD_1_8_1_to_1_1_3,1).^2)./mean(PSD_1_8_1,1)./mean(PSD_1_1_3,1));
set(gca,'Xscale','log')
xlabel('frequency [Hz]')
ylabel('magnitude coherence [-]')
